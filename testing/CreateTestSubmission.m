...
w2 = 400;
h2 = w2;
softmax = cell(numel(allImages),1);
if ~exist('softmax.mat', 'file')
    for imageIdx = 1:numel(allImages)
        disp(['image number : ', num2str(imageIdx)]);
        img = im2single(imread(fullfile(testDataFolder, allImages{imageIdx})));
        ...
        res = vl_simplenn(fishNet, croppedImg);
        softmax{imageIdx} = res(end).x;
    end
    save softmax softmax
    clear softmax;
end
load(fullfile(fileparts(currentPath), 'softmax.mat'));
softmaxMatrix = squeeze([softmax{:}]);
% softmaxMatrix = softmaxMatrix';
% [scores, predictions] = max(softmaxMatrix);
