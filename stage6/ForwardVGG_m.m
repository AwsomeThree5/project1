...
% load vgg-net
net = load(fullfile(vl_rootnnPath, 'imagenet-vgg-m-2048.mat'));
...
for imageIdx = 1 : numel(data.name)
    ...
    mkdir(fullfile(dataFolder,'scoreImagesVGG_m2048', data.fishType{imageIdx}));
    imageName = strsplit(data.name{imageIdx}, '.');
    imwrite(scoreImage,fullfile(dataFolder,'scoreImagesVGG_m2048', data.fishType{imageIdx}, [imageName{1},'.bmp']));
end
