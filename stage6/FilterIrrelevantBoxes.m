...
for boxIdx = 1:size(boxes, 1)
    ...
    elseif currentBox(3)/currentBox(4) > 3 || currentBox(4)/currentBox(3) > 3
        continue;
    % if the box is almost one dimensional
    elseif currentBox(4) < 30 || currentBox(3) < 30
        continue;
    elseif bboxOverlapRatio(currentBox, centroidBox, 'Min') < 0.4
        continue;
    else
...
disp([blanks(4),'additional filtering #1: ',num2str(size(boxes,1)), ' boxes']);
...
%         if overLap > 0.5 && (firstBoxSize < secondBoxSize) && goodBoxes(secondBoxIdx)
