...
% you need to run 'runKMeans' first.
% this function tries to look at the already unclustered images, and devide
% them to their right cluster, out of those already created by the function
% 'runKMeans'
% BE CAREFUL : it runs on every fish class manually
...
for imageIdx = 1:numel(unclusteredImages)
    ...
    for clusterIdx = 1:numOfClusters
        ...
        for imgIdx = 1:numOfImagesToRunOn
            ...
            subplot(1,2,2);
            imshow(clusterImg);
            % use siftflow or optical flow (or...?)
    ...
    similarityMatrix
    % display an image from the chose cluster
